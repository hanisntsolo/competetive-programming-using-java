    ...
    */
    ...
    public static void solveDirect() throws IOException {
        ...
        while(visitors-->0) {
        	int hour = fs.nextInt();
        	int min = fs.nextInt();
        	String key = String.valueOf(hour+"::"+min);
        	if(DEBUG){
        		out.print( key + "\n");
        	}
        	cacheMap.put(key, cacheMap.get(key) != null ? cacheMap.get(key) + 1 : 1);
        	totalCache = Math.max(totalCache, cacheMap.get(key));
        	if(DEBUG)
        		out.print( cacheMap + "\n");	
        }
        out.print( totalCache + "\n");
    }
