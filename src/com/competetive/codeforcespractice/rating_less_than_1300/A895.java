...
public class A895 {
 ...
 //    It's possible for multiple traps to occupy a single room (the values of di
 // can be repeated).
    public static void solve() {
        ...
        // int b = fs.nextInt();
        // int c = fs.nextInt();
        // pourAndCount(a, b, c, 0);
        // pour(a, b , c);
        takeValues(a);
    }
    public static void takeValues(int n) {
    	int[] trapPosAndTimer = new int[10000];
    	...
    	for(int i = 0; i < n; i++) {
    		...
    		int val = fs.nextInt();
    		if(flag) {
    			kMax = val % 2 == 0 ? pos + (val / 2 - 1) : pos + (val / 2) - 1;
    			flag = false;
    		}
    		trapPosAndTimer[pos] = val;
    	}
    	// out.print("KMAX" + kMax  + "\n");
    	for(int i = 1; i < 10000; i++) {
    		if(trapPosAndTimer[i] > 0) {
    			//check if you can move ahead or return ans
    			int timeLeftForNewTrap = trapPosAndTimer[i];
    			int actualTimeLeftToReturn = (kMax - i) * 2;
    			// out.print("i=" + i + "TIME"+ timeLeftForNewTrap +  "::"  + actualTimeLeftToReturn + "\n");
    			if(timeLeftForNewTrap > actualTimeLeftToReturn) {
    				if(timeLeftForNewTrap == actualTimeLeftToReturn - 1) continue;
    				continue;
    			}
    			else {
    				flag = true;
    ...
    // public static void trap(int n, int[][] trapAndTimer) {
    // 	//
    // 	// Solve
    ...
    // 	// out.print(countDown + "\n");
    // 	int currentRoom = 1;
    // 	int	countDown = trapAndTimer[0][1];
    // 	if(trapAndTimer[0][1] % 2 == 0) {
    // 		kMax = trapAndTimer[0][1] / 2 - 1;
    // 	} else {
    // 		kMax = trapAndTimer[0][1] / 2;
    // 	}
    // 	for(int room = 1; room <= countDown; room++) {
    // 		kMax
    // 	}
    // }
    public static void pourAndCount(int a, int b, int c, int count) {
    	out.print( "Pouring" + a + "::" + b + "Cup :: " + c + "\n");
    	if(a == b) {
    		out.print(count + "\n");
    		return;
    	}
    	...
    	//if a is greater try and equalize
    	if(a > b)
    		pourAndCount(a - c, b + c, c, count + 1);
    	if(a < b)
    		pourAndCount(a + c, b - c, c, count + 1);
    	//if b is greater try and equalize 
    }
    public static void pour(double a, double b , double c) {
    	...
    	if(diff == 0) {
    		...
    		double split =((a - b) / 2);
    		double take = (split / c);
    		// out.print( split + "::" + take + "\n");
    		if(take < 0)
    			out.print((int)Math.abs(Math.floor(take)) + "\n");
    		else
    			out.print((int)Math.abs(Math.ceil(take)) + "\n");
    	} 
