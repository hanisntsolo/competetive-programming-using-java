 // package com.competetive.codeforcespractice.rating_less_than_1300;// import java.util.*;
...
public class JEFFANDPERIODS {
     ...
     */
     ...
     //## while comparing characters make sure to enclose in single quotes.
      // final static boolean CP = true;
        final static boolean CP =false;
     static String inputFile;
     static String outputFile;
     public static void main(String[] args) {
         if(CP) {
             if(args.length < 2) {
                 System.out.println("Usage: Java JeffAndPeriods <>");
             }
             inputFile = args[0];
             outputFile = args[1];
             try {
                 // Redirect System.in to inputFile
                 FileInputStream fis = new FileInputStream(inputFile);
                 System.setIn(fis);

                 // Redirect System.out to outputFile
...
//                 long N = fs.nextLong();
     ...
     // 1 -> 0
     // 2 -> 0
     public stativ void solve() {
        String text = fs.next();
        List<Integer> abcd = new ArrayList<>(Collections.nCopies(26, 0));
        for(int i = 0 ; i < text.length(); i++ ){
            int index = Character.getNumericValue(text.charAt(i)) - 10;
            abcd.set(index, abcd.get(index) ^ 1);
        }
        int val = 0;
        for(int i = 0 ; i < abcd.size(); i++)
            val += abcd.get(i);
        }
        String result = (val > 1 && val % 2 == 1) ? "First" : "Second";
        out.print( result+ "\n");
     }
     // public static void solve() {
     //     int len = fs.nextInt();
     //     List<Integer> cDiff = new ArrayList<>(Collections.nCopies(100001, 0));
     //     List<Boolean> seen = new ArrayList<>(Collections.nCopies(100001, false));
     //     Map<Integer, Integer> previousDiff = new TreeMap<>();
     //     Map<Integer, Integer> previousIndex = new TreeMap<>();
     //     Map<Integer, Integer> map = new TreeMap<>();
     //     List<Integer> inValid = new ArrayList<>();
     //     for(int i = 0; i < len; i++) {
     //         int currNum = fs.nextInt();
     //         if(seen.get(currNum)) {
     //            map.put(currNum, map.get(currNum) + 1); // Count update
     //            int currDiff = i - previousIndex.getOrDefault(currNum, previousIndex.get(currNum));
     //            int commonDiffPrev = previousDiff.getOrDefault(currNum, 0);
     //            if(commonDiffPrev > 0 && currDiff == commonDiffPrev) {
     //                previousDiff.put(currNum, currDiff);
     //                cDiff.set(currNum, currDiff);
     //            } else if(commonDiffPrev > 0) {
     //                inValid.add(currNum);
     //            } else if(currDiff > 0) {
     //                previousDiff.put(currNum, currDiff);
     //                previousIndex.put(currNum, i);
     //            }
     //         } else {
     //            seen.set(currNum, true);
     //            previousIndex.put(currNum, i);
     //            map.put(currNum, 1);
     //         }
     //     }
     ...
     //         } else {
     //             sb.append(k).append(" ").append(cDiff.get(k) + "\n");
     //         }
     //     });
     //     out.print( map.size() + "\n");
...
//        Map<Integer, Integer> difference = new TreeMap<>();
//        Map<Integer, Boolean> valid = new TreeMap<>();
//
//        for (int i = 0; i < n; i++) {
...
//                int d = i - lastIndex.get(x);
//
//                if (difference.containsKey(x)) {
//                    if (difference.get(x) != d) {
//                        // Difference is inconsistent, remove from both maps
//                        lastIndex.remove(x);
//                        difference.remove(x);
//                    }
//                } else {
//                    // First time calculating difference for x
//                    difference.put(x, d);
//                }
//
//                // Update last index for x
//                if (lastIndex.containsKey(x)) {
//                    lastIndex.put(x, i);
//                }
//            }
...
//        for (Map.Entry<Integer, Boolean> entry : valid.entrySet()) {
//            if (entry.getValue()) {
//                int x = entry.getKey();
//                int d = difference.getOrDefault(x, 0);
