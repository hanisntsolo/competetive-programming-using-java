...
public class PoloThePenguinAndMatrix {
    ...
    */
    ...
    public static void solveDirect() throws IOException {
        ...
        int medianIndex1 = nums.size() > 2 ? ((nums.size()) / 2) + 1 : (((nums.size()) - 1) / 2) + 1;
        int medianIndex2 = nums.size() > 2 ? (nums.size()) / 2 : (nums.size() - 1) / 2;
        int median1 = nums.get(medianIndex1);
        int median2 = nums.get(medianIndex2);
        if(!possible) {
        	out.print( -1 + "\n");
        } else {
        	if(DEBUG) {
        		out.print( "median1 : " + median1 + "\n");
        		out.print( "median2 : " + median2 + "\n");
        	}
        	int count1 = 0;
        	int count2 = 0;
        	for(int i = 0 ; i < nums.size(); i++) {
        		if(nums.get(i) != median1) {
        			count1 += (Math.abs(median1 - nums.get(i)) / diff); 
        			if(DEBUG) {
        				out.print( count1  + "\n");
        			}
        		}
        		if(nums.get(i) != median2) {
        			count2 += (Math.abs(median2 - nums.get(i)) / diff); 
        			if(DEBUG) {
        				out.print( count2  + "\n");
        			}
        		}
        	}
        	out.print( Math.min(count1, count2) + "\n");
        }
    ...
    /**
     * vertex would be 1 leftSegment = 0, rightSegment = len - 1 
     */
    static class BuildSegmentTreeForSumRangeQuery {
        int n;
        List<Integer> segmentTree;
        public BuildSegmentTreeForSumRangeQuery(int n) {
            this.n = n;
            this.segmentTree = new ArrayList<>(Collections.nCopies(4 * n, 0));
        }
        public void build(List<Integer> arr, int vertex, int leftSegment, int rightSegment) {
            if(leftSegment == rightSegment) {
                segmentTree.set(vertex, arr.get(leftSegment));
            } else {
                int middle = (leftSegment + rightSegment) / 2;
                build(arr, vertex * 2, leftSegment, middle);
                build(arr, vertex * 2 + 1, middle + 1, rightSegment);
                segmentTree.set(vertex, segmentTree.get(vertex * 2) + segmentTree.get(vertex * 2 + 1));
            }
        }
        public int sum(int vertex, int leftSegment, int rightSegment, int leftQuery, int rightQuery) {
            if(leftQuery > rightQuery) {
                return 0;
            }
            if(leftQuery == leftSegment && rightQuery == rightSegment) {
                return segmentTree.get(vertex);
            }
            int middle = (leftSegment + rightSegment) / 2;
            return sum(vertex * 2, leftSegment, middle, leftQuery, Math.min(rightQuery, middle))
            + sum(vertex * 2 + 1, middle + 1 , rightSegment, Math.max(leftQuery, middle + 1), rightQuery);
        }
        public void update(int vertex, int leftSegment, int rightSegment, int position, int newValue) {
            if(leftSegment == rightSegment) {
                segmentTree.set(vertex, newValue);
            } else {
                int middle = (leftSegment + rightSegment) / 2;
                if(position <= middle) {
                    update(vertex * 2, leftSegment, middle, position, newValue);
                } else {
                    update(vertex * 2 + 1, middle + 1, rightSegment, position, newValue);
                }
                segmentTree.set(vertex, segmentTree.get(vertex * 2) + segmentTree.get(vertex * 2 + 1));
            }
        }
        public List<Integer> getSegmentTree() {
            return this.segmentTree;
        }
    }
    /**
     * Calculate totalFactors of a given Integer
     * @params n.
     */ 
    ...
    /**
     * Calculate pythagorean triplets upto N.
     * @params int
     */
