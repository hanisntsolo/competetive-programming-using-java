...
public class A882 {
    ...
    */
    ...
    public static void solve() {
        ...
        long[] bufferArr = new long[len];
        long[] zeroArr = new long[len];
        long res = 0;
        for(int i = 0; i < len; i++) {
        	long currentNum  = fs.nextLong();
        	inputArr[i] = currentNum;
        	if(currentNum < 0)
        		bufferArr[i] = Math.abs(currentNum);
        	else
        		bufferArr[i] = currentNum;
        	res+=bufferArr[i];
        	zeroArr[i] = inputArr[i] + bufferArr[i];
        }
        int zeroTracker = 1;
        int flipTracker = 0;
        for(int i = 0; i < len; i++) {
        	if(zeroArr[i] == 0 && inputArr[i] < 0) 
        		zeroTracker = 0;
        	if(zeroArr[i] > 0 && zeroTracker == 0) {
        		flipTracker += 1;
        		zeroTracker = 1;
        	}
        }
