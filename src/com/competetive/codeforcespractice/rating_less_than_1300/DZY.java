...
public class DZY {
    ...
    */
    ...
    public static Character[][] placePieces(Character[][] grid, int m, int n) {
    	for (int i = 0 ; i < m ; i++) {
    		for (int j = 0 ; j < n ; j++) {
    			//if a good position then check surroundings
    			...
    			for (int d = 0 ; d < directions.length ; d++) {
    				...
    				if(nextRow > grid.length - 1 ||
    				 nextCol > grid[0].length - 1 ||
    				  nextRow < 0 || nextCol < 0) continue;
    				// if(grid[nextRow][nextCol] == '-') continue;
    				//Check whether surrounding contain chess piece
    				//if yes then place the opposing piece
    				if(grid[nextRow][nextCol] == 'W' 
    					&& grid[currentRow][currentCol] == '.'
    					// || grid[currentRow][currentCol] == 'W'
    				) {
