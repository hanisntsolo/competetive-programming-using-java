...
public class Ribbon {
    ...
    */
    ...
    public static void solveDirect() throws IOException {
    	...
    	int b = fs.nextInt();
    	int c = fs.nextInt();
    	List<Integer> dp = new ArrayList<Integer>(Collections.nCopies(length + 1, Integer.MIN_VALUE));
    	dp.set(0,0);
    	for(int i = a; i <= length; i++) {
    		dp.set(i, Math.max(dp.get(i), dp.get(i - a) + 1));
    	}
    	for(int i = b; i <= length; i++) {
    		dp.set(i, Math.max(dp.get(i), dp.get(i - b) + 1));
    	}
    	for(int i = c; i <= length; i++) {
    		dp.set(i, Math.max(dp.get(i), dp.get(i - c) + 1));
    	}
    	out.print(dp.get(length) + "\n");
    }
