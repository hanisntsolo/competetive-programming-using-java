...
public class RankList {
    ...
    */
    ...
    static class Team implements Comparable<Team> {
	    ...
	    public Team(int score, int penalty) {
	        this.score = score;
	    ...
	    public int compareTo(Team team) {
	        if(score > team.score) {
	            ...
	            return 1;
	        } else if(score == team.score && penalty < team.penalty) {
	            return -1;
	        } else if(score == team.score && penalty > team.penalty) {
	            return 1;
	        } else {
	    ...
	    public String toString() {
	    	return "[Score:" + this.score + "," +"Penalty:" + this.penalty + "]";
	    }
	}
    ...
    public static void solveDirect() throws IOException {
        ...
        int rankNum = fs.nextInt();
        for(int i = 0; i < teams; i++) {
        	teamList.add(new Team(fs.nextInt(), fs.nextInt()));
        }
        Collections.sort(teamList);
        // out.print( teamList + "\n");
        int[] rank = new int[51];
        int rankIndex = 0;
        int prevScore = teamList.get(0).getScore();
        int prevPenalty = teamList.get(0).getPenalty();
        rank[rankIndex] = 1;
        for(int i = 1; i < teamList.size(); i++) {
        	int currScore = teamList.get(i).getScore();
        	int currPenalty = teamList.get(i).getPenalty();
        	if(prevScore == currScore && prevPenalty == currPenalty) {
        		rank[rankIndex]++;
        	...
        	prevScore = currScore;
        	prevPenalty = currPenalty;
        }
        int[] finalRank = new int[51];
        int l = 0;
        int x = 0;
        while(x < finalRank.length) {
        	int totalTeam = rank[l];
        	if(totalTeam == 0)
        		break;
        	while(totalTeam-- > 0) {
        		finalRank[x] = rank[l];
        ...
        // out.print( Arrays.toString(finalRank) + "\n");
        // out.print( Arrays.toString(rank) + "\n");
        out.print( finalRank[rankNum-1] + "\n");
    }
