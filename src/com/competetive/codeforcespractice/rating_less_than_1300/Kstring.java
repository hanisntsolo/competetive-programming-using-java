    ...
    public static void solve(int k) throws IOException {
    	...
    	for(Map.Entry<Character, Integer> m : map.entrySet()) {
    		if(m.getValue() % k != 0) {
    			...
    			sb.append(multiplyString(m.getKey().toString(), m.getValue() / k));
    		}
    	}
        //Compare the input k with the count of chars if yes
        //then print the k string base_string*k;
    	out.print(multiplyString(sb.toString(), k)  + "\n");
