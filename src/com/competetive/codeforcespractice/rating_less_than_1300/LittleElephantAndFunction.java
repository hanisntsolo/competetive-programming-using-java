...
public class LittleElephantAndFunction {
	...
	public static void main(String[] args) throws IOException {
		...
		swap(totalNumbers, permutation);
		permutation.stream().map(ele -> ele+" ").forEach(System.out::print);
	}
	public static void swap(int n, ArrayList<Integer> permutation) {
		if(n == 1) return;
		int temp = permutation.get(n - 1);
		permutation.set(n - 1, permutation.get(n - 2));
		permutation.set(n - 2, temp);
		swap(n - 1, permutation);
	}
    static class Reader {
