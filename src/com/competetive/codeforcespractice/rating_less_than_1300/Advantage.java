    ...
    */
    ...
    public static void main(String[] args) throws IOException {
        ...
        while(test > 0) {
        	...
        	// for(long p : list) { list2.add(p);}
        	while(n > 0) {
        		long strength = fs.nextLong();
        		if(!map.containsKey(strength)) {
        			map.put(strength, (long)1);
        		} else {
        	...
        	// out.print( list + "\n");
        	// out.print( list2 + "\n");
        	HashMap<Long, Long> longList = new HashMap<>();
        	for(int i = 0 ; i < list.size() - 1; i++) {
        		longList.put(list.get(i), list.get(i) - list.get(list.size() - 1));
        	}
        	longList.put(list.get(list.size() - 1), list.get(list.size() - 1) - list.get(list.size() - 2));
        	//Mapping the list with their correct presentations.
        	HashMap<Long, Long> map2 = new HashMap<>();
        	for(int i = 0; i < list2.size(); i++) {
        		long currNum = list2.get(i);
        		// out.print( currNum + " ");
        		// out.print( longList + "\n");
        		map2.put(currNum, longList.get(currNum));
        	}
        	// out.print("\n");
        	long index = 0;
        	for(long k : map.keySet()) {
        		...
        		try {
        		while(iter > 0) {
	        		out.print( map2.get(list2.get((int)index++)).toString() + " ");
	        		iter--;
        		}
