...
public class A894 {
    ...
    */
    ...
    public static void solve() {
        ...
        int n = fs.nextInt();
        int m = fs.nextInt();
        char[][] vals = new char[n][m];
        for(int i = 0 ; i < n; i++) {
            vals[i] = fs.next().toCharArray();
        }
        char[] vika = {'v', 'i', 'k', 'a'};
        int k = 0;
        for(int j = 0; j < m; j++) {
            String check = "";
            for(int i = 0; i < n; i++) {
                check+=String.valueOf(vals[i][j]);
            }
            if(check.contains(String.valueOf(vika[k]))) {
                k+=1;
            }
            if(k == 4) {
                break;
        ...
        // out.print( Arrays.deepToString(vals) + "\n");
    }
