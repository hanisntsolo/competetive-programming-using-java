...
public class RestoreThePermutation {
    ...
    */
    ...
    public static void solve() {
        ...
        Arrays.sort(unUsedInt);
        for(int i = n / 2 - 1; i >= 0; i--) {
            int k = upper_bound(unUsedInt, p[2 * i + 1]);
            if(k == unUsed.iterator().next()) {
                out.print( "-1" + "\n");
                return;
            }
            k--;
...
//kind of binaryArray.
static int recursive_upper_bound(int arr[], int low, int high, int key) {
...
static int upper_bound(int arr[], int key) {
    int low = 0;
    int high = arr.length;
    int upperBound = recursive_upper_bound(arr, low, high, key);
    if(upperBound == arr.length) return -1;
    return upperBound;
}
