    ...
    */
    ...
    public static void solveDirect() throws IOException {
    	...
    	for(int i = 0 ; i < total; i++) {
    		long currNum = fs.nextLong();
    		list.add(currNum);
    	}
        List<Long> primeList = PrimeSieve.generatePrimes2((int)max).stream().map(entry -> entry * entry).toList();
        if(DEBUG==true) {
        	out.print( primeList + "\n");
        }
        for(int i = 0; i < list.size(); i++) {
	        if(primeList.contains(list.get(i))) {
	        	out.print( "YES" + "\n");
	        } else {
	        	out.print( "NO" + "\n");
	        }
    ...
    static public class PrimeSieve {
	  ...
	  public static List<Long> generatePrimes2(int n) {
	    List<Long> primes = new ArrayList<>();
	    List<Boolean> primeSieve = new ArrayList<>(Collections.nCopies(n + 1, true));
	    primeSieve.set(0, false);
	    primeSieve.set(1, false);
	    for(int i = 2; i <= n; i++) {
	      if(primeSieve.get(i) != null) {
	        primes.add((long)i); // Add that result to the resulting array.
	      }
