...
public class ConsecutiveSubsequence {
    ...
    */
    ...
    public static void solve() {
		...
		int[] a = new int[(int)2e5 + 20];
		int len = fs.nextInt();
		int ans = 0;
		int curr = 0;
		for (int i = 0; i < len ; i++ ) {
			int currElement = fs.nextInt();
			a[i] = currElement;
			map.put(currElement, Math.max(map.getOrDefault(currElement, 0), map.getOrDefault(currElement - 1, 0) + 1));
			if(map.get(currElement) > ans) {
				ans = map.get(currElement);
				curr = currElement;
			}
		}
		out.print( ans + "\n");
