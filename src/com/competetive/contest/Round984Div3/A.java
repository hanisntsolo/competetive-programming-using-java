...
public class A {
    ...
    */
    ...
    static HashSet<Integer> suka = new HashSet<>();
    static HashSet<Integer> kayo = new HashSet<>();
    ...
    public static void printYesOrNo() {
        int tones = fs.nextInt();
        int currentTone = fs.nextInt();
        boolean flag = false;
        while(tones-- > 1) {
            int nextTone = fs.nextInt();
            if(Math.abs(nextTone - currentTone) != 5 && Math.abs(nextTone - currentTone) != 7) {
                flag = true;
            }
