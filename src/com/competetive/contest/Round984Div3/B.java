...
public class B {
    ...
    */
    ...
    static class Bottle {
        int bottleBrandIndex;
        int bottleCost;
        public Bottle(int brandIndex, int cost) {
            this.bottleBrandIndex = brandIndex;
            this.bottleCost = cost;
        }
        @Override
        public String toString() {
            return "[ brandIndex : "+ bottleBrandIndex + " cost : " + bottleCost+ "]";
        }
    }
    public static void solveDirect() throws IOException {
        ...
        while(test-- > 0) {
            int n = fs.nextInt();
            int k = fs.nextInt();
            calculateRevenue(n, k);
        }
    }
    public static void calculateRevenue(int shelves, int bottleCount) {
        Queue<Bottle> bottleQueue = new PriorityQueue<>(
            (o1, o2) -> o2.bottleCost != o1.bottleCost ? Integer.compare(o2.bottleCost, o1.bottleCost) : Integer.compare(o1.bottleBrandIndex, o2.bottleBrandIndex)
        );
        HashMap<Integer, Integer> bottleMap = new HashMap<>(); 
        while(bottleCount-- > 0) {
            int idx = fs.nextInt();
            int cost = fs.nextInt();
            bottleQueue.add(new Bottle(idx, cost));
            if(!bottleMap.containsKey(idx)) {
                bottleMap.put(idx, cost);
            } else {
                bottleMap.put(idx, bottleMap.get(idx) + cost);
            }
        }
        long maxProfit = 0;
        if(bottleMap.size() <= shelves) {
            for(Map.Entry<Integer, Integer> entry: bottleMap.entrySet()) {
                maxProfit += entry.getValue();
            }
        } else if(shelves < bottleMap.size()) {
        List<Map.Entry<Integer, Integer>> sortedEntries = bottleMap.entrySet()
            .stream()
