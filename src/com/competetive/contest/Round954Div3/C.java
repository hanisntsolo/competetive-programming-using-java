    ...
    */
    ...
    public static void solveDirect() throws IOException {
        ...
        while(test-- > 0) {
            ...
            computeQueries(input, queries);
        }
    }
    public static void computeQueries(String input, int queries) {
        StringBuilder stringBuilder = new StringBuilder(input);
        int inputLength = stringBuilder.length();
        int matchCount = 0;

        for (int i = 0; i <= inputLength - 4; i++) {
            if (stringBuilder.substring(i, i + 4).equals("1100")) {
                matchCount++;
            }
        }

        while (queries-- > 0) {
            int index = fs.nextInt(); // Read the index
            int val = fs.nextInt();   // Read the value (0 or 1)

            char newChar = (char) (val + '0');
            char oldChar = stringBuilder.charAt(index - 1);

            if (newChar != oldChar) {
                for (int i = Math.max(0, index - 4); i <= Math.min(inputLength - 4, index); i++) {
                    if (stringBuilder.substring(i, i + 4).equals("1100")) {
                        matchCount--; // Remove old match if it exists
                    }
                }

                stringBuilder.setCharAt(index - 1, newChar);

                for (int i = Math.max(0, index - 4); i <= Math.min(inputLength - 4, index); i++) {
                    if (stringBuilder.substring(i, i + 4).equals("1100")) {
                        matchCount++; // Add new match if it exists
                    }
                }
            }
