        ...
        for(int i = 0 ; i < matrix.length; i++) {
            for(int j = 0; j < matrix[0].length; j++) {
                ...
                for(int k = 0; k < directions.length; k++) {
                    int nextRow = directions[k][0] + currRow;
                    int nextCol = directions[k][1] + currCol;
                    if(nextCol >= 0 && nextRow >= 0 && nextCol < matrix[0].length && nextRow < matrix.length) {
                        if(matrix[nextRow][nextCol] > max) {
                            max = matrix[nextRow][nextCol];
                        }
                    }
                }
                matrix[currRow][currCol] = max == -1 ? matrix[currRow][currCol] : Math.min(matrix[currRow][currCol], max);
            }
        }
        printMatrix(matrix);
    }
