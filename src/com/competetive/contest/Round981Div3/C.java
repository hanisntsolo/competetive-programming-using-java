...
public class C {
    ...
    */
    ...
    public static void solveDirect() throws IOException {
        ...
        while(tests-- > 0) {
            int size = fs.nextInt();
            int[] currentStudents = takeStudent(size);
            int currentDisturbance = totalDisturbance(currentStudents);
            if(currentDisturbance == 0) {
                out.print( 0 + "\n");
            } else {
                int start = 0;
                int end = currentStudents.length - 1;
                while(start < end) {
                    swap(currentStudents, start, end);
                    currentDisturbance = Math.min(currentDisturbance, totalDisturbance(currentStudents));
                    swap(currentStudents, start, end);
                    start++;
                    end--;
                }
                out.print( currentDisturbance + "\n");
            }
        }
    }
    ...
    public static int[] takeStudent(int size) {
        int[] students = new int[size];
        for(int i = 0 ; i < size; i++) 
            students[i] = fs.nextInt();
        return students;
    }
    public static int totalDisturbance(int[] arr) {
//          5
// //          2 1 2 2 1 2
//                       e
//                       s
        ...
        while(end < arr.length) {
            ...
            if(end == arr.length - 1 && arr[end] == arr[start] && end != start) {
                disturbance+=1;
            }
            end++;  
        }
