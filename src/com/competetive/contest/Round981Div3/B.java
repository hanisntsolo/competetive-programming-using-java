...
public class B {
    ...
    */
    ...
    public static void solveDirect() throws IOException {
        ...
        while(test-- > 0) {
            int size = fs.nextInt();
            out.print(createMountain(readMatrix(size)) + "\n");
        }
    }
    ...
    public static int createMountain(int[][] matrix) {
        int net = 0;
        for(int i = matrix.length - 1; i >= 0; i--) {
            ...
            while(row <= matrix.length - 1 && col <= matrix.length - 1) {
                if(DEBUG)
                    out.print( "x : " + row +"col : " + col + "\n");
                if(matrix[row][col] < 0) {
                    if(DEBUG)
                        out.print( "PREVIOUS : " + currentMin + "NEXT : " + matrix[row][col] + "\n");
                    currentMin = Math.min(matrix[row][col], currentMin);
                }
                row+=1;
                col+=1;
            }
            if(currentMin < 0) {
                net += Math.abs(currentMin);
            } 
        }
