    ...
    */
    ...
    public static void solveDirect() throws IOException {
        ...
        for(int i = 0; i < test; i++) {
            int notes = fs.nextInt();
            StringBuilder sb = new StringBuilder("");
            for(int j = 0; j < notes; j++) {
                String note = fs.next();
                sb.insert(0, (note.indexOf("#") + 1) + " ");
            }
            out.print( sb.toString() + "\n");
        }
    }
    private static void print(int len, String matStr) {
    	int sqroot = (int)Math.sqrt(len);
    	if(sqroot * sqroot != len) {
    		out.print( "NO" + "\n");
    		return;
    	}
    	for(int i = 1; i < sqroot - 1; i++) {
    		for(int j = 1; j < sqroot - 1; j++) {
    			if(matStr.charAt(i * sqroot + j) == '1') {
    				out.print( "NO" + "\n");
    				return;
				}
			}
