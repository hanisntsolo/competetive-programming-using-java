...
// 3 8 | 2 6 // 4
    ...
    //Since Suneet and Slavic aren't best friends, you need to calculate the number of ways the 
    //game could happen that Suneet would end up as the winner.
// 1 0 1 0  10-> win then can win the match
// 0 1 0 1  05
...
// 3 8 2 6
    ...
    // Check all 4 possible game scenarios and count the number of times Suneet wins.
    if ((nums[0] > nums[2] && nums[1] > nums[3])) { 
        // Suneet wins both rounds using a1 vs b1 and a2 vs b2
        suneetWins++;
    }
    if ((nums[0] > nums[3] && nums[1] > nums[2])) { 
        // Suneet wins both rounds using a1 vs b2 and a2 vs b1
        suneetWins++;
    }
    if ((nums[1] > nums[2] && nums[0] > nums[3])) { 
        // Suneet wins both rounds using a2 vs b1 and a1 vs b2
        suneetWins++;
    }
    if ((nums[1] > nums[3] && nums[0] > nums[2])) { 
        // Suneet wins both rounds using a2 vs b2 and a1 vs b1
        suneetWins++;
    }
        out.print(suneetWins + "\n");
    }
