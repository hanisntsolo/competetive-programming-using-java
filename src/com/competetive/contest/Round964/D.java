...
public class D {
    ...
    */
    ...
    /**
     * 
     * Slavic is asked to change each "?" to a lowercase 
     * English letter such that string t
...
.
    ...
    public static void solve() {
        ...
        while(pt < t.length && ps < s.length) {
            if(ps == s.length && pt != t.length - 1) {
                out.print( "NO" + "\n");
                return;
            }
            if(s[ps].equalsIgnoreCase(t[pt])) {
                pt++;
            }
            if(s[ps].equalsIgnoreCase("?")) {
                s[ps] = t[pt];
                pt++;
                ps++;
            } else {
