...
public class Tprime {
    ...
    */
    ...
    public static void solveDirect() throws IOException {
    	...
    	int primes = 1000_000_000;
        List<Long> list = PrimeSieve.generatePrimes2(primes);
        if(list.contains(fs.nextLong())) {
        	out.print( "YES" + "\n");
        } else {
        	out.print( "NO" + "\n");
        }
	  ...
	  public static List<Long> generatePrimes2(int n) {
	    ...
	    for(int i = 2; i <= n; i++) {
	      if(primeSieve.get(i) != null) {
	        primes.add((long)i*i); // Add that result to the resulting array.
	      }
